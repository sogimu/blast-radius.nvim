...
function M.get_recent_changes(files, opts, callback)
  ...
  -- Determine git root from the first file's directory, not nvim's cwd
  local first_file_dir = vim.fn.fnamemodify(vim.fn.fnamemodify(files[1], ":p"), ":h")
  local git_root = vim.fn.system({ "git", "-C", first_file_dir, "rev-parse", "--show-toplevel" }):gsub("%s+", "")
  ...
  -- Normalize: ensure git_root ends with /
  local git_root_prefix = git_root:sub(-1) == "/" and git_root or (git_root .. "/")
  ...
  for _, f in ipairs(files) do
    ...
    if abs:sub(1, #git_root_prefix) == git_root_prefix then
      local rel = abs:sub(#git_root_prefix + 1)
      table.insert(rel_files, rel)
      file_lookup[rel] = true
    end
  end

  if vim.tbl_isempty(rel_files) then
    vim.notify("blast-radius.nvim: no files inside git repo", vim.log.levels.WARN)
    vim.schedule(function() callback({}) end)
    return
  end
  ...
  local function on_batch_done(err, batch_changes)
    if err and err ~= "" then
      vim.notify("blast-radius.nvim: git error: " .. err, vim.log.levels.ERROR)
    end
    if batch_changes then
    ...
    vim.system(cmd, { cwd = git_root }, function(result)
      ...
      if result.stdout and result.stdout ~= "" then
        ...
        for line in result.stdout:gmatch("[^\r\n]+") do
          ...
          if h then
            current_hash, current_date, current_author, current_msg = h, d, a, m
